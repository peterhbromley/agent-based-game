...
to create-walls
  ...
  while [num-wall-groups > 0] [
    let wall-start one-of patches with [ valid-wall-patch ]
    ask wall-start [ create-wall-group (random (num-walls-max - num-walls-min) + num-walls-min)  num-wall-groups ]
    set num-wall-groups num-wall-groups - 1
  ]
end
...
to init-start-and-end
  set start-patch one-of patches with [pxcor = min-pxcor and abs pycor > (max-pycor - (max-pycor / 2))]
